...
import ai.djl.huggingface.tokenizers.OSGiSafeHuggingFaceTokenizer;
...
/**
 * Original file from
 * https://github.com/langchain4j/langchain4j-embeddings/blob/0.35.0/langchain4j-embeddings/src/main/java/dev/langchain4j/model/embedding/onnx/OnnxBertBiEncoder.java
 */

public class OSGiSafeOnnxBertBiEncoder  {
    ...
    private final OSGiSafeHuggingFaceTokenizer tokenizer;
    private final PoolingMode poolingMode;

    public OSGiSafeOnnxBertBiEncoder(InputStream model, InputStream tokenizer, PoolingMode poolingMode) {
        try {
            this.environment = OrtEnvironment.getEnvironment();
            this.session = environment.createSession(loadModel(model));
            this.expectedInputs = session.getInputNames();
            this.tokenizer = OSGiSafeHuggingFaceTokenizer.newInstance(tokenizer, singletonMap("padding", "false"));
    ...
    public OSGiSafeOnnxBertBiEncoder(OrtEnvironment environment, OrtSession session, InputStream tokenizer, PoolingMode poolingMode) {
        try {
