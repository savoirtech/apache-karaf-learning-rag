...
public class OSGiSafePlatform {
    ...
    /** Constructor used only for {@link OSGiSafePlatform#fromSystem()}. */
    private OSGiSafePlatform() {}
     ...
     */
    public static OSGiSafePlatform detectPlatform(String engine, String overrideVersion) {
        OSGiSafePlatform platform = OSGiSafePlatform.fromSystem(engine);
     ...
     * @return the platform that matches current operating system
     */
    public static OSGiSafePlatform detectPlatform(String engine) {
        String nativeProp = "native/lib/" + engine + ".properties";
        Enumeration<URL> urls;
        try {
            urls = OSGiSafePlatform.class.getClassLoader().getResources(nativeProp);
        } catch (IOException e) {
            throw new AssertionError("Failed to list property files.", e);
        }

        OSGiSafePlatform systemPlatform = OSGiSafePlatform.fromSystem(engine);
        OSGiSafePlatform placeholder = null;
        while (urls.hasMoreElements()) {
            URL url = urls.nextElement();
            OSGiSafePlatform platform = OSGiSafePlatform.fromUrl(url);
     ...
     */
    static OSGiSafePlatform fromUrl(URL url) {
        OSGiSafePlatform platform = OSGiSafePlatform.fromSystem();
     ...
     */
    public static OSGiSafePlatform fromSystem(String engine) {
        String engineProp = engine + "-engine.properties";
        String versionKey = engine + "_version";
        OSGiSafePlatform platform = fromSystem();
        platform.placeholder = true;

        try {
            URL url = OSGiSafePlatform.class.getClassLoader().getResource(engineProp);
            if (url != null) {
     ...
     */
    static OSGiSafePlatform fromSystem() {
        OSGiSafePlatform platform = new OSGiSafePlatform();
     ...
     */
    public boolean matches(OSGiSafePlatform system) {
