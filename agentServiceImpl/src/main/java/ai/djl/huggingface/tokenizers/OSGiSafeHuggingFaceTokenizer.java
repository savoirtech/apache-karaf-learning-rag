...
import com.savoir.apache.karaf.rag.agent.service.impl.OSGiSafeLibUtils;
...
public class OSGiSafeHuggingFaceTokenizer extends NativeResource<Long> implements Tokenizer {

    private static final Logger logger = LoggerFactory.getLogger(OSGiSafeHuggingFaceTokenizer.class);
    ...
    private OSGiSafeHuggingFaceTokenizer(long handle, Map<String, String> options) {
        super(handle);
    ...
    /**
     * Creates a pre-trained {@code OSGiSafeHuggingFaceTokenizer} instance from huggingface hub.
     *
     * @param name the name of the huggingface tokenizer
     * @return a {@code OSGiSafeHuggingFaceTokenizer} instance
     */
    public static OSGiSafeHuggingFaceTokenizer newInstance(String name) {
        return newInstance(name, null);
    }

    /**
     * Create a pre-trained {@code OSGiSafeHuggingFaceTokenizer} instance from huggingface hub.
     *
     * @param identifier the identifier of the huggingface tokenizer
     * @param options tokenizer options
     * @return a {@code OSGiSafeHuggingFaceTokenizer} instance
     */
    public static OSGiSafeHuggingFaceTokenizer newInstance(String identifier, Map<String, String> options) {
        ...
        return new OSGiSafeHuggingFaceTokenizer(handle, options);
    }

    /**
     * Create a pre-trained {@code OSGiSafeHuggingFaceTokenizer} instance from existing models.
     *
     * @param modelPath the directory or file path of the model location
     * @return a {@code OSGiSafeHuggingFaceTokenizer} instance
     * @throws IOException when IO operation fails in loading a resource
     */
    public static OSGiSafeHuggingFaceTokenizer newInstance(Path modelPath) throws IOException {
     ...
     * @throws IOException when IO operation fails in loading a resource
     */
    public static OSGiSafeHuggingFaceTokenizer newInstance(Path modelPath, Map<String, String> options)
            throws IOException {
    ...
    /**
     * Create a pre-trained BPE {@code OSGiSafeHuggingFaceTokenizer} instance from existing models.
     *
     ...
     * @throws IOException when IO operation fails in loading a resource
     */
    public static OSGiSafeHuggingFaceTokenizer newInstance(
            Path vocab, Path merges, Map<String, String> options) throws IOException {
        Ec2Utils.callHome("Huggingface");
        OSGiSafeLibUtils.checkStatus();
    ...
    /**
     * Create a pre-trained {@code OSGiSafeHuggingFaceTokenizer} instance from {@code InputStream}.
     *
     ...
     * @throws IOException when IO operation fails in loading a resource
     */
    public static OSGiSafeHuggingFaceTokenizer newInstance(InputStream is, Map<String, String> options)
            throws IOException {
        Ec2Utils.callHome("Huggingface");
        OSGiSafeLibUtils.checkStatus();
    ...
    /**
     * Creates a builder to build a {@code OSGiSafeHuggingFaceTokenizer}.
     *
    ...
    public static final class Builder {
         ...
         */
        private OSGiSafeHuggingFaceTokenizer managed(OSGiSafeHuggingFaceTokenizer tokenizer) {
            if (manager != null) {
         ...
         * @throws IOException when IO operation fails in loading a resource
         */
        public OSGiSafeHuggingFaceTokenizer build() throws IOException {
            String tokenizerName = options.get("tokenizer");
            if (tokenizerName != null) {
                return managed(OSGiSafeHuggingFaceTokenizer.newInstance(tokenizerName, options));
            }
            ...
            if (Files.isDirectory(tokenizerPath)) {
                ...
                if (Files.exists(tokenizerFile)) {
                    return managed(OSGiSafeHuggingFaceTokenizer.newInstance(tokenizerPath, options));
                }
                ...
                if (Files.exists(vocab) && Files.exists(merges)) {
                    return managed(OSGiSafeHuggingFaceTokenizer.newInstance(vocab, merges, options));
                }
